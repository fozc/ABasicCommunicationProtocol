...
#define FRAME_START 0x01 ///0x01
#define FRAME_END   0x04 //0x04
#define FRAME_DLE   0x10 //0x04
...
char my_memcmp(const void *Ptr1, const void *Ptr2, size_t Count)
{
    char v = 0;
    char *p1 = (char *) Ptr1;
    char *p2 = (char *) Ptr2;
...
void serial_rx_process(void)
{
    ...
    if (memcmp(serial.buffer, info, sizeof (info)) == 0)
    {
        LED1_();
        reset_serial_com();
        return;
